from tennis_photos import get_player_photo
"""
...
# Sort by Grand Slam titles descending, then by rating descending
ATP_PLAYERS.sort(key=lambda p: (-p["grand_slams"], -p["rating"]))
# Re-number display rank based on original ATP rank (keep rank field as ATP rank)
# But rebuild rank-lookup after sort
ATP_BY_RANK = {p["rank"]: p for p in ATP_PLAYERS}
